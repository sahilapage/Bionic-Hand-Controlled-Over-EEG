...
# Human finger extension range (palm-relative, metres).
# HUMAN_Z_MIN: fully curled fingertip distance from palm (~0.03-0.05m)
# HUMAN_Z_MAX: fully extended fingertip distance from palm (~0.17-0.19m)
# ext_ratio = (human_Z - Z_MIN) / (Z_MAX - Z_MIN) maps the full range to [0,1].
# Increase Z_MIN if fist is not curling enough; decrease if open hand over-extends.
HUMAN_Z_MIN  = 0.05
HUMAN_Z_MAX  = 0.18

# Simulation smoothing
CURL_GAMMA   = 1.5   # Power curve for curl emphasis (>1 = tighter fist)
THUMB_X_MAX  = 0.10  # Thumb lateral range: negative X = tucked, positive = spread
...
# Human lateral spread range (thumb-to-pinky, metres).
# Larger value = less sideways bending on the robot.
# 0.25 keeps m2 subtle; reduce toward 0.10 for more pronounced spread.
HUMAN_X_MAX = 0.40
...
def retarget_tsv(tsv: np.ndarray,
    ...
    m1 (flexion) ← human Z = -human_Y (extension signal)
        Maps human_Z ∈ [0, HUMAN_Z_MAX] to robot Z range [z_curl, z_ext] per finger,
        then inverts through the calibrated Jacobian to get motor angle.
        Works for ALL fingers including thumb — no sign branching needed.

    m2 (abduction) ← human X (lateral spread)
        Simple proportional map: human_X / HUMAN_X_MAX * π/2
    """
    ...
    for fi, hi in enumerate(HUMAN_ORDER):
        v = remap(tsv[hi])
        human_z = v[2]   # extension: large = finger far from palm = extended
        human_x = v[0]   # lateral spread

        is_thumb = (fi == 3)   # finger4 = thumb, different anatomy

        if not is_thumb:
            # ── Fingers 1-3: m1 = flexion/extension via Z ────────────────────
            # Linear extension ratio then apply power curve to push curl harder
            ext_ratio = float(np.clip(
                (human_z - HUMAN_Z_MIN) / (HUMAN_Z_MAX - HUMAN_Z_MIN), 0.0, 1.0))
            ext_ratio = ext_ratio ** CURL_GAMMA   # gamma>1 emphasises curl

            # Map to robot Z range, invert through Jacobian
            target_z = z_curl[fi] + ext_ratio * (z_ext[fi] - z_curl[fi])
            m1 = (target_z - tips0[fi][2]) / jac[fi]["m1"][2]

            # m2 = lateral spread (kept small via HUMAN_X_MAX)
            m2 = human_x / HUMAN_X_MAX * CTRL_HIGH

        else:
            # ── Thumb: curl happens across the palm (X axis) ─────────────────
            # Negative human X = thumb tucked across palm = curl (high m1)
            # Positive human X = thumb spread outward    = extend (low m1)
            m1 = -(tsv[hi][0] / THUMB_X_MAX) * CTRL_HIGH

            # m2 = small depth correction from raw HAMER Z
