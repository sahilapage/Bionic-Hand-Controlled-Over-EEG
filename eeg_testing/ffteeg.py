...
class EEGMonitor(QMainWindow):
    ...
    def update_brainpower_plot(self):
        ...
        if band_powers is not None:
            ...
            print(f"\rDelta: {band_powers['delta']*100:5.1f}% | "
                f"Theta: {band_powers['theta']*100:5.1f}% | "
                f"Alpha: {band_powers['alpha']*100:5.1f}% | "
                f"Beta: {band_powers['beta']*100:5.1f}% | "
                f"Gamma: {band_powers['gamma']*100:5.1f}%", end="")
