"""Retarget a human hand pose onto the Amazing Hand, in MuJoCo.

    python -m sohand.retarget.retarget                       # demo poses
    python -m sohand.retarget.retarget --tsv-folder poses/
    python -m sohand.retarget.retarget --tsv-file pose.npy

Input is a Task Space Vector: a (5, 3) array of fingertip positions relative to
the palm, as produced by `sohand.perception.hand_pose`. The representation is
from DexMV (Qin et al., CVPR 2022).

The mapping is a per-finger Jacobian inverse, measured from the model at
startup rather than hardcoded: each motor is swept to +-0.5 rad and the tip
displacement recorded. This matters because the fingers are passive four-bar
linkages -- their constrained pose only resolves after physics settles, so
`mj_forward` alone leaves every fingertip at its unconstrained position. That
was the original cause of "the motors do not move".
"""
...
# In mjcf/hand the actuators are named after the joints they drive.
...
def remap_frame(v: np.ndarray) -> np.ndarray:
    """HAMER frame -> robot frame.

    HAMER puts the fingers along -Y; the robot puts them along +Z. So
    `robot_Z = -human_Y` becomes the extension signal and `robot_X = human_X`
    the lateral-spread signal.
    """
...
def get_tip_vecs(model, data, palm_id, site_ids) -> np.ndarray:
    """Palm-relative fingertip positions, in the order of `TIP_SITES`."""
...
def run_simulation(tsv_files, model_path, hold_time=2.0):
    ...
    site_ids = [model.site(n).id for n in TIP_SITES]
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if step_count >= steps_per_hold:
                ...
                print(f"[SIM] -> {pose_idx + 1}/{len(retargeted)}: "
                      f"{retargeted[pose_idx][0]}")
...
def main():
    ...
    p.add_argument("--tsv-folder", default=None)
    p.add_argument("--hold-time", type=float, default=2.0)
    global HUMAN_Z_MAX
    p.add_argument("--human-z-max", type=float, default=HUMAN_Z_MAX,
                   help="max human fingertip extension, m (default 0.18); "
                        "lower it for a small hand")
    args = p.parse_args()
    ...
    if args.tsv_file:
        ...
        # Three real HAMER outputs -- a loose hand, a partial fist and an open
        # spread -- so the viewer shows the mapping working without a dataset.
        print("[INFO] Demo mode: three sample poses.\n")
        samples = {
            "loose_hand": np.array([
                [-0.0353, -0.0869, -0.0025], [0.0103, -0.0698, 0.0131],
                [0.0002, -0.0593, 0.0118], [-0.0121, -0.0395, 0.0133],
                [-0.0291, -0.0321, 0.0076]]),
            "partial_fist": np.array([
                [-0.0545, -0.0664, -0.0392], [-0.0034, -0.1708, -0.0046],
                [-0.0626, -0.1626, 0.0106], [-0.0147, -0.0412, -0.0171],
                [-0.0272, -0.0351, -0.0158]]),
            "open_spread": np.array([
                [0.0980, -0.0786, -0.0357], [0.0120, -0.1714, 0.0042],
                [-0.0361, -0.1704, 0.0177], [-0.0690, -0.1462, 0.0290],
                [-0.1043, -0.0946, -0.0016]]),
        }
        tmp = tempfile.mkdtemp(prefix="tsv_demo_")
        for name, arr in samples.items():
            path = os.path.join(tmp, f"{name}_tsv.npy")
            np.save(path, arr)
