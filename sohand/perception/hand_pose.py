"""Extract MANO parameters, 3D joints and Task Space Vectors from RGB images.

    python -m sohand.perception.hand_pose --img-folder frames/ --out-folder poses/
    python -m sohand.perception.hand_pose --img-folder photo.jpg --save-mesh

Pipeline: MediaPipe Hands finds the hands and their handedness, HAMER
(Pavlakos et al.) regresses a MANO hand from each crop, and the Task Space
Vector is the palm-to-fingertip displacement for all five fingers -- a (5, 3)
array, the representation DexMV (Qin et al., CVPR 2022) uses to move a human
pose onto a robot hand. `sohand.retarget` consumes exactly that array.

Per detected hand this writes `{image}_{id}_mano.npy`, `{image}_{id}_tsv.npy`,
optionally `{image}_{id}.obj`, and one `{image}_overlay.png` per image.

HAMER is not on PyPI; install it from https://github.com/geopavlakos/hamer.
"""
...
# MANO joint indices: the wrist, then one fingertip per finger.
PALM_ID = 0
...
def compute_tsvs(joints_3d):
    """Task Space Vectors: palm-to-fingertip displacements, shape (5, 3)."""
    palm = joints_3d[PALM_ID]
    return np.stack([joints_3d[tip] - palm for tip in FINGERTIP_IDS])

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--checkpoint', type=str, default=DEFAULT_CHECKPOINT)
    parser.add_argument('--img-folder', type=str, required=True)
    parser.add_argument('--out-folder', type=str, default='demo_out')
    ...
    for img_path in tqdm(img_paths):
        ...
        for batch in loader:
            ...
            for n in range(B):
                ...
                mano_params = {
                    "global_orient": mano["global_orient"][n].cpu().numpy(),
                    "hand_pose": mano["hand_pose"][n].cpu().numpy(),
                    "betas": mano["betas"][n].cpu().numpy(),
